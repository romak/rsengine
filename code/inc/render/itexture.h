...
namespace rengine3d {
	...
	public:
		...
		textureType_t GetType(void)				{return m_type;};
		...
		void SetType(textureType_t type)		{m_type = type;};
	...
	r_inline ITexture::ITexture():m_height(0),m_width(0),m_depth(0),m_bpp(0),m_type(textureType_Normal),m_target(textureTarget_2D),m_wrapS(textureWrap_Repeat),
		m_wrapT(textureWrap_Repeat),m_wrapR(textureWrap_Repeat),m_filter(textureFilter_Trilinear),m_anisotropyDegree(1.0f),m_useMipMaps(false),m_useCompress(false) {
	}
