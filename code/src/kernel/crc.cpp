...
namespace rengine3d {
	...
	const int  STRING_POOL_SIZE = 1024 * 8 - 256;

	static uint randBits[] = {
	...
	uint MakeCRC (const char* const name) {
		if (!name) {
			return 0;
		}
		...
		for (int i = 0; name[i]; i ++) {
			char c = name[i];
			uint val = randBits[((crc >> 24) ^ c) & 0xff];
			crc = (crc << 8) ^ val;
		}
	...
	uint MakeCRC (const void* const buffer, int size, uint crcAcc){
		unsigned char* const ptr = (unsigned char*)buffer;
		for (int i = 0; i < size; i ++) {
			uint c = ptr[i];
			uint val = randBits[((crcAcc >> 24) ^ c) & 0xff];
			crcAcc = (crcAcc << 8) ^ val;
		}
		return crcAcc;
	}
